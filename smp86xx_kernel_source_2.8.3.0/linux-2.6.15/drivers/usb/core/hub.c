...
static void EHSET_tests(void *data)
{
	...
	/*??? For testing  purpose: uncomment following line 
	                            and select different test*/
	//udev->descriptor.idProduct = EHSET_TEST_PACKET;
...
int usb_new_device(struct usb_device *udev)
{
	...
	usb_notify_add_device(udev);
       ...
       if (udev->speed == USB_SPEED_HIGH &&
                       udev->parent == udev->bus->root_hub &&
                       udev->descriptor.idVendor == /*0x067B*/0x1A0A)/*??? Replace with test device Vendor ID*/ 
       {
